...
{
    CreateWindow(WindowName, location(20, 20), size(1800, 800), colour(white), SDL_WINDOW_RESIZABLE);
    log("Window Created");
    fflush(stdout);
    ColourArea = new ResizableArea(location(0), size(800, 120), colour(60), colour(white), 4, SizeLock::height, *renderer);
    log("Colour area created");
    CanvasArea = new ResizableArea(location(0, 160), size(800, 600 - ((*ColourArea).getSize().height * 2)), colour(128), colour(white), 0, SizeLock::width, *renderer);
